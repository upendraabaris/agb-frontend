...
function ItemTraking({ tracking }) {
  ...
  return (
    <>
      {tracking && (
        <div className="container m-1 pb20">
          <div className="row">
              ...
              <div className={`order-tracking ${tracking.shipped && 'completed'}`}>
                ...
                <p>
                  ...
                  {tracking.shippedImage && (
                    <>
                      <br />
                      {tracking.shippedDate && <span>{moment(parseInt(tracking.shippedDate, 10)).format('DD-MMM-yyyy')}</span>}
                      <br />
                      <Button variant="link" className="btn btn-link p-0 sh-3" onClick={() => downloadImage(tracking.shippedImage, 'Shipped Image')}>
                        <span className="small">Receipt Image</span>
                      </Button>
                    </>
                  )}
                  {/* {tracking.shippedBy && (
                    <>
                      <br />
                      <span> Shipped By: {tracking.shippedBy}</span>
                    </>
                  )}
                  {tracking.trackingNo && (
                    <>
                      <br />
                      <span> Tracking No: {tracking.trackingNo}</span>
                    </>
                  )}
