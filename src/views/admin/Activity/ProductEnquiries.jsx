...
function ListEnquire() {
  const title = 'Single Product Enquiry';
  const description = 'Single Product Enquiry List';
    ...
    <>
        ...
        cartEnquiries?.map((enquiry) => (
          <Card key={enquiry.id} className="mb-1 single-line-card">
            <div className="p-3 row m-0">
              <div className="col-3">
                <div className="fw-semibold">{enquiry.customerName}</div>
              ...
              <div className="col-3">{enquiry.message}</div>

              <div className="col-4">
                {enquiry.productName &&
                  enquiry.productName.split('<br />').map((name, i) => (
                    <div key={i}>
                      {' '}
                      {i + 1} {name.trim()}
                    </div>
      ...
      <Modal centered scrollable show={editModal} onHide={() => setEditModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Single Product Enquiry</Modal.Title>
        </Modal.Header>
