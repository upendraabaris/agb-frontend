...
function AboutUs({ data, theme, modules, eventKey, refetch }) {
  ...
  return (
    <>
      <h2 className="small-title my-2 py-2">About Us</h2>
      {/* {data && <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(data.content) }} className="mb-3 fs-5" />} */}
      ...
      {/* {data  && 
          <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(data.content) }} className="mb-3 fs-5" />}
        <div className="mb-3 filled form-group tooltip-end-top">
            <ReactQuill modules={modules} theme="snow" placeholder="Compose an epic Blog" value={mainContent} onChange={setMainContent} name="content" />
          </div>
         <Button onClick={() => handleEditContent()}>Save Changes</Button>  */}
