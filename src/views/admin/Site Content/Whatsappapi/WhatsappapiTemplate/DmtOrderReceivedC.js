...
function DmtOrderReceivedC() {
  const title = 'DMT Order Received Customer';
  const description = 'DMT Order Received Customer';  
  const history = useHistory();
  const eventKey = 'dmtOrderReceivedCustomer';
  ...
  <HtmlHead title={title} description={description} />
    ...
    <div className="mb-3 filled form-group tooltip-end-top">
      <div className='small'>(First Name : $firstname, Mobile No.: $mobile, Order Id: $orderid, Email: $email, Order Amount: $amount)</div>
      <Form.Control
          className="p-2 border"
          type="text"
          value={mainContent}
          onChange={(e) => setMainContent(e.target.value)}
          placeholder="DMT Order Received Customer"
        />
...
export default DmtOrderReceivedC;
