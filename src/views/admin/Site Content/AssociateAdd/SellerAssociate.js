...
function SellerAssociate({ title, data, eventKey, refetch }) {
  const [sellerassociate, setSellerAssociate] = useState(''); // Correct state variable
  ...
  async function handleSave() {
    try {
      await updateContent({
        variables: {
          key: eventKey,
          content: sellerassociate, // Use the correct state variable here
        },
      });
    } catch (error) {
  ...
  useEffect(() => {
    setSellerAssociate(data?.content || '');
  }, [data]);
    ...
    <>
      <Card className="hover-border-primary mx-0 my-0 px-0 py-0">
        <Card.Body className="mx-4 my-4 px-0 py-0">
          ...
          <Form.Control
            type="text"
            value={sellerassociate} // Correct state variable here
            onChange={(e) => setSellerAssociate(e.target.value)} // Correct setter function
          />
