...
function BusinessAssociate({ title, data, eventKey, refetch }) {
  const [businessAssociateDes, setBusinessAssociateDes] = useState(''); 
  ...
  useEffect(() => {
    setBusinessAssociateDes(data?.content || '');
  }, [data]);
    ...
    <>
      <Card className="hover-border-primary mx-0 my-0 px-0 py-0">
        <Card.Body className="mx-4 my-4 px-0 py-0">
          ...
          <Form.Control
            type="text"
            value={businessAssociateDes}
            onChange={(e) => setBusinessAssociateDes(e.target.value)} 
          />
