...
function OrderPacksms() {
  const title = 'Order Pack SMS Customer';
  const description = 'Order Pack SMS Customer ';  
  const history = useHistory();
  const eventKey = 'orderpacksms';
  ...
  const [editcontent, { data: dataSite }] = useMutation(ADD_SITE_CONTENT, {
    onCompleted: () => {
      toast(`Order pack sms is updated.`);
      refetch();
    },
    onError: (error) => {
