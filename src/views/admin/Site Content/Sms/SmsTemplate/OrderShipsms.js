...
function OrderShipsms() {
  const title = 'Order Shipping SMS Customer';
  const description = 'Order Shipping SMS Customer ';  
  const history = useHistory();
  const eventKey = 'orderShipping';
  ...
  const [editcontent, { data: dataSite }] = useMutation(ADD_SITE_CONTENT, {
    onCompleted: () => {
      toast(`Order shipping sms is updated.`);
      refetch();
    },
    onError: (error) => {
