...
function OrderDeliveryCustomer() {
  const title = 'Order Delivery SMS';
  const description = 'Ecommerce Order Delivery SMS Page';  
  const history = useHistory();
  const eventKey = 'orderDeliverysms';
  ...
  const [editcontent, { data: dataSite }] = useMutation(ADD_SITE_CONTENT, {
    onCompleted: () => {
      toast(`Order delivery sms is updated.`);
      refetch();
    },
    onError: (error) => {
  ...
  return (<>
    <HtmlHead title={title} description={description} />
      ...
      <div className="mb-3 filled form-group tooltip-end-top">
        <div className='small'>(First Name: $firstname, Mobile No.: $mobile, Order Amount: $amount, Order Id: $orderid)</div>
      <Form.Control
          className="p-2 border"
          type="text"
          value={mainContent}
          onChange={(e) => setMainContent(e.target.value)}
          placeholder="Order Delivered SMS"
        />
...
export default OrderDeliveryCustomer;
