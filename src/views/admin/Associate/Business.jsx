...
const GET_ALL_SELLERS = gql`
  query GetAllSuperSellers($search: String, $limit: Int, $offset: Int, $sortBy: String, $sortOrder: String) {
    getAllSuperSellers(search: $search, limit: $limit, offset: $offset, sortBy: $sortBy, sortOrder: $sortOrder) {
      id
      email
      companyName
      companyDescription
  ...
  return (
    <>
      ...
      <Row className="m-0 mb-2 p-1 rounded bg-white align-items-center">
        <Col md="6">
          <span className="fw-bold fs-5 ps-2 pt-2">BA List</span>
          <span className="small ps-2">{data && data.getAllSuperSellers ? `(${data.getAllSuperSellers.length})` : ''}</span>
        </Col>
        ...
        <Table bordered hover className="align-middle bg-white">
          <thead>
            <tr>
              <th className="cursor-pointer sort" onClick={() => handleSort('companyName')}>
                BA Name <CsLineIcons icon="filter" width="12px" />
              </th>
              ...
              <th className="cursor-pointer sort" onClick={() => handleSort('gstin')}>
                GST Number <CsLineIcons icon="filter" width="12px" />
              </th>
          ...
          <tbody>
            {loading ? (
              ...
              <>
                {data?.getAllSuperSellers?.length > 0 ? (
                  data.getAllSuperSellers.map((seller) => (
                    <tr key={seller.id}>
                      ...
                      <td className="text-center">
                        <NavLink to={`/admin/associate/businessDetail/${seller.id}`}>
                          <CsLineIcons icon="navigate-diagonal" width="12px" />
                        </NavLink>
                      </td>
                    </tr>
                  ))
      ...
      {data && (
        <div className="d-flex justify-content-center mt-5">
          <Pagination>
            ...
            <Pagination.Next className="shadow" onClick={() => handlePageChange(offset + limit)} disabled={data.getAllSuperSellers?.length < limit}>
