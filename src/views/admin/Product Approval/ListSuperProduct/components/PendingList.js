...
function PendingList({ setPendingCount }) {
  ...
  const [Pendingsupersellerproduct, { fetchMore, data, refetch }] = useLazyQuery(PENDING_APPROVAL_PRODUCT, {
    onError: (error) => {
  ...
  useEffect(() => {
    setPendingCount(data?.pendingsupersellerproduct?.filter((product) => !product.reject === true).length || 0);
                  ...
                  <Row className="g-0 h-100 sh-lg-9 position-relative">
                    <Col xs="1" className="position-relative h-100">
                      <NavLink to="#/!" className="d-block h-100">
                        <img src={product.thumbnail || (product.images.length && product.images[0])} alt="product" className="border rounded mh-100" />
                      </NavLink>
                    </Col>
                    <Col className="py-4 py-lg-0 ps-5 pe-4 h-100">
                      <Row className="g-0 h-100 align-content-center">
                        <Col xs="11" lg="3" className="d-flex flex-column mb-lg-0 mb-3 pe-3 d-flex order-1 h-lg-100 justify-content-center">
                          <NavLink to="#/!">
                            {product.previewName}
                            <div className="text-small text-muted text-truncate">{product.superSellerId}</div>
                          </NavLink>
                        </Col>
                            ...
                            <OverlayTrigger placement="top" overlay={<Tooltip id="tooltip-top">Product Details</Tooltip>}>
                              <div className="d-inline-block me-2">
                                <Button
                                  ...
                                  onClick={() => {
                                    history.push(`detailsSuperProduct/${product.identifier?.replace(/\s/g, '_').toLowerCase()}`);
                                  }}
                                >
