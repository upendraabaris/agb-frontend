...
function DetailSeller() {
  const title = 'Dealer Associate Profile';
  ...
  const handleSubmit = async () => {
    const newErrors = {};
    if (!company.trim()) newErrors.company = 'Dealer Name is required.';
    ...
    if (!desc.trim()) newErrors.desc = 'Dealer Description is required.';
      ...
      {data && (
        <>
          ...
          <div className="page-title-container mb-2">
            <Row className="align-items-center">
              <Col className="col-auto d-flex align-items-center">
                <NavLink className="text-decoration-none d-flex align-items-center me-2" to="/subBusiness/dashboard">
                  <span className="fw-medium text-dark">Dashboard</span>
                </NavLink>
                <span className="text-dark">/</span>
                <NavLink className="text-decoration-none d-flex align-items-center ms-2 me-2" to="/subBusiness/profile">
          ...
          <Row className="m-0 mb-3 p-1 rounded bg-white align-items-center">
            <Col md="6">
              <span className="fw-bold text-dark fs-5 ps-2 pt-2">{title}</span>
            </Col>
          </Row>
          <Row>
            <Col xl="4">
              <Card className="mb-2 pb-4">
                <Card.Body className="mb-n5 p-3">
                  ...
                  <div className="mb-3 ">
                    ...
                    <h4 className="mt-4 pt-4 fs-6 border-top fw-bold text-dark">Your BA Name</h4>
                    <ul className="p-0 list-unstyled">
                      {data.getSeller.superSellerId.map((seller, sIndex) => (
                        <li key={sIndex} className="text-dark pb-1">
                          {sIndex + 1}. {seller.companyName}
                          <div className="float-end">
                            {data.getSeller.allotted.map((allot, aIndex) => (
                              <div key={aIndex} className="text-dark ps-2">
                                <a
                                  href="#"
                                  className="text-primary text-decoration-underline"
                                  onClick={(e) => {
                                    e.preventDefault();
                                    openPincodeModal(seller);
                                  }}
                                >
                                  View Pincodes
                                </a>
            ...
            <Col xl="8">
              <Card className="mb-1 shadow-sm">
                <Card.Body className="p-4">
                  <h4 className="mb-3 fw-bold text-dark">Dealer Associate Information</h4>
                  <Row className="gy-3">
                    <Col md={6}>
                      <strong>Dealer Associate Name:</strong>
                    ...
                    <Col md={12}>
                      <strong>Dealer Description:</strong>
          ...
          <Modal className="scroll-out-negative" show={editModal} onHide={() => setEditModal(false)} scrollable dialogClassName="modal-lg">
            <Modal.Header closeButton className="pt-3 pb-3">
              <Modal.Title as="h4" className="fw-bold">
                Edit Dealer Associate Information
              </Modal.Title>
            </Modal.Header>
                  ...
                  <div className="row">
                    <div className="mb-2 col-md-6 col-12">
                      <Form.Label className="fw-bold text-dark">
                        Dealer Associate Name <span className="text-danger"> *</span>
                      </Form.Label>
                    ...
                    <div className="mb-2">
                      <Form.Label className="fw-bold text-dark">
                        Dealer Associate Description <span className="text-danger"> *</span>
                      </Form.Label>
          ...
          <Modal show={showPincodeModal} onHide={closePincodeModal} centered size="md">
            <Modal.Header closeButton className="p-3">
              <Modal.Title className="fw-bold text-dark">Allotted Pincode Details</Modal.Title>
            </Modal.Header>

            <Modal.Body className="overflow-auto p-3" style={{ maxHeight: '60vh' }}>
              {data?.getSeller?.allotted
                // eslint-disable-next-line no-underscore-dangle, dot-notation
                ?.filter((allot) => allot.superSellerId === selectedSeller?.['_id'])
                .map((allot, index) => (
                  <div key={index} className="p-3 mb-3 border rounded shadow-sm" style={{ backgroundColor: '#f9f9f9' }}>
                    <div className="mb-2">
                      <strong>Your BA Name:</strong> <span className="text-primary">{selectedSeller?.companyName}</span>
                    </div>

                    {Array.isArray(allot.state) && allot.state.length > 0 ? (
                      <div className="mb-2">
                        <strong>Alloted States:</strong> <span className="text-success">{allot.state.join(', ')}</span>
                      </div>
                    ) : (
                      <div>
                        <div className="mb-2">
                          <strong>Alloted Pincodes ({allot.pincode.length})</strong>
                        </div>
                        <div className="p-2 border rounded bg-white text-break">
                          {Array.isArray(allot.pincode) &&
                            allot.pincode.map((pin, idx) => (
                              <span key={idx}>
                                {pin}
                                {idx !== allot.pincode.length - 1 && ', '}
                              </span>
                            ))}
                        </div>
                      </div>
              ...
              {data?.getSeller?.allotted
                // eslint-disable-next-line no-underscore-dangle, dot-notation
                ?.filter((allot) => allot.superSellerId === selectedSeller?.['_id']).length === 0 && (
                <div className="text-center text-muted py-5">No pincodes allotted</div>
              )}
            </Modal.Body>

            <Modal.Footer>
              <Button variant="secondary" onClick={closePincodeModal}>
                Close
              </Button>
            </Modal.Footer>
          </Modal>
