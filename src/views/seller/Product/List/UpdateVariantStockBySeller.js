...
// 🔹 Query for fetching products and variants
...
const UpdateVariantStocksBySeller = () => {
  const title = 'Bulk Stock Update';
  const description = 'Update Main and Display Stock by Seller';
  ...
  // 🔹 Mutation (fixed name)
  const [updateStocks, { loading: updating }] = useMutation(UPDATE_VARIANT_PRICES, {
    onCompleted: () => toast.success('Stock updated successfully!'),
  ...
  const handleFieldChange = (productId, variantId, locationId, field, value) => {
    setStockUpdates((prev) => {
      ...
      updated[productId][variantId][locationId][field] = parseInt(value, 10) || 0;
      return updated;
    });
  };

  // 🔹 Save All Stocks for a Product
  ...
  const handleSave = async (product) => {
    try {
      ...
      const variantsData = product.variant.map((variant) => ({
        ...
        locations: variant.location.map((loc) => ({
          id: loc.id,
          mainStock: stockUpdates?.[product.id]?.[variant.id]?.[loc.id]?.mainStock ?? loc.mainStock ?? 0,
          displayStock: stockUpdates?.[product.id]?.[variant.id]?.[loc.id]?.displayStock ?? loc.displayStock ?? 0,
        })),
      }));
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="d-flex gap-2 flex-wrap align-items-center">
                            {[
                              { label: 'Main Stock', field: 'mainStock' },
                              { label: 'Display Stock', field: 'displayStock' },
                            ].map(({ label, field }) => (
                              <div key={field} style={{ width: '45%' }}>
                                ...
                                <Form.Control
                                  ...
                                  value={stockUpdates?.[product.id]?.[variant.id]?.[loc.id]?.[field] ?? loc[field] ?? ''}
...
export default UpdateVariantStocksBySeller;
