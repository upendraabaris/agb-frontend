...
const UpdateVariantPricesBySeller = () => {
  const title = 'Bulk Price Update';
  const description = 'Bulk Price Update by Seller';
  ...
  const [updatePrices, { loading: updating }] = useMutation(UPDATE_VARIANT_PRICES, {
    onCompleted: () => toast.success('Prices updated successfully!'),
    onError: (error) => toast.error(error.message || 'Failed to update prices!'),
  });
  ...
  // 🔹 Handle Field Changes (price, extraCharge, transportCharge)
  ...
  const handleSave = async (product) => {
    ...
    const variantsData = product.variant.map((variant) => {
      const locData = variant.location.map((loc) => ({
        id: loc.id,
        price: priceUpdates?.[product.id]?.[variant.id]?.[loc.id]?.price ?? loc.price ?? 0,
        extraCharge: priceUpdates?.[product.id]?.[variant.id]?.[loc.id]?.extraCharge ?? loc.extraCharge ?? 0,
        transportCharge: priceUpdates?.[product.id]?.[variant.id]?.[loc.id]?.transportCharge ?? loc.transportCharge ?? 0,
      }));
      return {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="d-flex gap-2 flex-wrap align-items-center">
                            {[
                              { label: `${loc.priceType || 'N/A'}`, field: 'price' },
                              { label: `${loc.extraChargeType || 'N/A'}`, field: 'extraCharge' },
                              { label: `${loc.transportChargeType || 'N/A'}`, field: 'transportCharge' },
                            ].map(({ label, field }) => (
                              <div key={field} style={{ width: '30%' }}>
                                <Form.Label className="small text-muted mb-1 d-block">{label}</Form.Label>
                                <Form.Control
                                  type="number"
                                  step="0.01"
                                  min="0"
                                  className="form-control-sm rounded-1 shadow-none border-light p-1"
                                  value={priceUpdates?.[product.id]?.[variant.id]?.[loc.id]?.[field] ?? loc[field] ?? ''}
