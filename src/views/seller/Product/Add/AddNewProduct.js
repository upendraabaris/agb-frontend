...
function AddNewProduct({ history }) {
  const title = 'Add Product without Variant';
      ...
      Promise.all(checks).then(() => {
        if (invalidFound || validFiles.length !== selectedFiles.length) {
          ...
          setFormErrors((prevErrors) => ({
            ...prevErrors,
            [name]: 'Please upload only square images (equal width and height).',
          }));
          setFormData((prevFormData) => ({
            ...prevFormData,
            [name]: [],
        ...
        <div>
          ...
          <div className="p-1">
            <Card className="border">
              <div className="mark">
                <div className=" fw-bold ps-4 p-1 fs-6">Basic Detail</div>{' '}
              </div>
