...
import { Accordion, Row, Col, Form, Card, Button, Tooltip, OverlayTrigger } from 'react-bootstrap';
...
const VarientPicker = ({ setFormData1, handleChange }) => {
  ...
  const handlePincodeChange = (variantIndex, locationIndex, pincodeValues, stateNames = []) => {
    setFormData((prevState) => {
      const updatedLocation = {
        ...prevState.variant[variantIndex].location[locationIndex],
        pincode: pincodeValues,
        state: stateNames, // 🔹 change "states" → "state"
      };
      const updatedVariant = {
        ...prevState.variant[variantIndex],
        location: prevState.variant[variantIndex].location.map((location, index) => (index === locationIndex ? updatedLocation : location)),
  ...
  const handleSubmit = async (event) => {
    ...
    formData.variant.forEach((variant, variantIndex) => {
      ...
      const { deliveryType, selectedStates, tags } = variant;

      if (deliveryType === 'state' && (!selectedStates || selectedStates.length === 0)) {
        newErrors[`deliveryType-${variantIndex}`] = 'Please select at least one state';
        if (!firstErrorElement) {
          firstErrorElement = document.querySelector('input[name="deliveryType"][value="state"]');
        }
      }

      if (deliveryType === 'manual' && (!tags || tags.length === 0)) {
        newErrors[`deliveryType-${variantIndex}`] = 'Please enter at least one pincode';
        if (!firstErrorElement) {
          firstErrorElement = document.querySelector('input[name="deliveryType"][value="manual"]');
        }
      }
            ...
            <div className="p-1 mt-2">
              <Form onSubmit={handleSubmit}>
                ...
                {formData.variant.map((variant, variantIndex) => (
                  <>
                    {/* <div className="mark border pb-2">
                      <div className="fw-bold ps-3 p-1 fs-6 pb-0">
                        Other Details{' '}
                        <OverlayTrigger
                          placement="top"
                          overlay={
                            <Tooltip id="tooltip-top" className="custom-tooltip">
                              Enter other details for the product such as HSN, GST Rate, Price etc.
                            </Tooltip>
                          }
                        >
                            ...
                            {finalPricedata && (
                              <div className=" col-md-6 mb-2">
                                <Form.Group controlId={`variant[${variantIndex}].location[${locationIndex}].finalPrice`}>
                                  <Form.Label className="fw-bold text-dark">
                                    Select Final Price Type<span className="text-danger"> * </span>{' '}
                                    <OverlayTrigger
                                      placement="top"
                                      overlay={
                                        <Tooltip id="tooltip-top" className="custom-tooltip">
                                  ...
                                  <div className="mb-2">
                                    <Form.Group controlId={`variant[${variantIndex}].location[${locationIndex}].pincode`}>
                                      <DetailAttributeItem
                                        ...
                                        onPincodeChange={(pincodeValues, stateNames) => handlePincodeChange(variantIndex, locationIndex, pincodeValues, stateNames)}
                                        ...
                                        onAllPincodeChange={(isAllIndia) => {
                                          setFormData((prev) => {
                                            ...
                                            return {
                                              ...prev,
                                              variant: prev.variant.map((v, i) => (i === variantIndex ? updatedVariant : v)),
                                            };
                                          });
                                        }}
                                        formData={formData}
                  ...
                  <div key={variantIndex}>
                    ...
                    {!variant.allPincode && (
                      <OverlayTrigger
                        placement="top"
                        overlay={
                          <Tooltip id={`tooltip-${variantIndex}`} className="custom-tooltip">
