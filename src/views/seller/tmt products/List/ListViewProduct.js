...
const ListViewProduct = ({ history }) => {
  ...
  const handleSelect = (key) => {
    if (key === "section_false") {
      history.push("/seller/tmt_product/list");
    } else if (key === "section_true") {
  ...
  return (
    <>
      ...
      >
        <Tab eventKey="section_false" title="Regular Series Products (Fixed Variants)">
          {filteredSectionFalse.length > 0 ? (
            ...
            <div className="text-center text-muted py-5">Product not found</div>
          )}
        </Tab>

        <Tab eventKey="section_true" title="Relational Series Products (Fixed Variants)">
          {filteredSectionTrue.length > 0 ? (
            <SectionDiffTrueList
